  ...
  <div class="p-10">
    <div v-if="tvShowStore.tvShowData" class="flex gap-10 max-lg:flex-col">
      <a
        ...
        class="relative h-fit max-w-64 max-lg:mx-auto lg:max-w-60"
      >
        <div
          class="lef-0 absolute top-0 z-1 h-full w-full opacity-20 duration-300 ease-in-out hover:rounded hover:bg-black"
        ></div>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex max-h-[75vh] flex-col gap-2 overflow-y-auto pr-2">
          <template v-for="(episode, index) in tvShowStore.tvShowData.episodes" :key="index">
            <EpisodeListItem :data="episode" />
          </template>
        </div>
      </div>
    </div>
    <div v-else>It was not possible to obtain the tv show data</div>
  </div>
</template>
